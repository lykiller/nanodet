...
class RepShuffleV2Block(nn.Module):
    def __init__(self, inp, oup, stride, activation="ReLU", deploy=False):
        super(RepShuffleV2Block, self).__init__()
...
class RepShuffleNetV2(nn.Module):
    def __init__(
        self,
        model_size="1.5x",
        out_stages=(2, 3, 4),
        ...
        super(RepShuffleNetV2, self).__init__()
        # out_stages can only be a subset of (2, 3, 4)
        assert set(out_stages).issubset((2, 3, 4))
        ...
        for name, repeats, output_channels in zip(
            ...
            seq = [
                RepShuffleV2Block(
                    input_channels, output_channels, 2, activation=activation, deploy=deploy
                )
            ]
            for i in range(repeats - 1):
                seq.append(
                    RepShuffleV2Block(
                        output_channels, output_channels, 1, activation=activation, deploy=deploy
                    )
                )
            setattr(self, name, nn.Sequential(*seq))
            input_channels = output_channels
