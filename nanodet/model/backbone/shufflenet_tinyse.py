...
class ShuffleNetTinySe(nn.Module):
    def __init__(
        self,
        model_size="0.75x",
        out_stages=(2, 3),
        ...
        super(ShuffleNetTinySe, self).__init__()
        # out_stages can only be a subset of (2, 3)
        assert set(out_stages).issubset((2, 3))

        print("model size is ", model_size)

        self.stage_repeats = [6, 3]
        ...
        if model_size == "0.5x":
            self._stage_out_channels = [24, 48, 96, 512]
        elif model_size == "0.75x":
            self._stage_out_channels = [24, 88, 176, 512]
        elif model_size == "1.0x":
            self._stage_out_channels = [24, 116, 232, 512]
        elif model_size == "1.5x":
            self._stage_out_channels = [24, 176, 352, 512]
        elif model_size == "2.0x":
            self._stage_out_channels = [24, 232, 464, 1024]
        else:
        ...
        # self.stage2 = ShuffleV2Block(output_channels, self._stage_out_channels[1], 2, activation=activation)
        ...
        if self.with_last_conv:
            ...
            self.stage3.add_module("conv5", conv5)
