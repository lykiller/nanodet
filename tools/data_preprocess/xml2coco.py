...
def xml2json(img_dir, xml_dir, target_dir, ratio):
    ratio = float(ratio)
    make_dir(target_dir)
    ...
    train_nums = math.floor(len(xml_list) * ratio)
    xml_list_train = xml_list[:train_nums]
    xml_list_test = xml_list[train_nums:]

    train_anno = convert(img_dir, xml_list_train, label2id)
    val_anno = convert(img_dir, xml_list_test, label2id)
    with open(osp.join(target_dir, 'train.json'), 'w', encoding='utf-8') as f:
        f.write(json.dumps(train_anno))
    with open(osp.join(target_dir, 'val.json'), 'w', encoding='utf-8') as f:
        f.write(json.dumps(val_anno))


if __name__ == '__main__':
    train_img_dir = "/home/user/work/dataset/df_bsd/imgs"
    train_xml_dir = "/home/user/work/dataset/df_bsd/tiny_xml"
    target_dir = "/home/user/work/dataset/df_bsd/annotations"

    xml2json(train_img_dir, train_xml_dir, target_dir, 0.8)
